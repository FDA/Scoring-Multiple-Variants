...
def parse_neutral(genename, accid, email=Entrez.email, conservative=True):
	Entrez.email=email
	if conservative:
...
def pipeline_internal(internal=internal, email=Entrez.email):
	data = {}
...
def pipeline_manual(runset=runset, ntseq=None, email=Entrez.email
...
def pipeline(genename, disid, accid, email=Entrez.email
