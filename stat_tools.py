...
def correlation_lists(x, y, forbidden=["", None], threshold = 3, mode="sm"):
	x = list(x)
	y = list(y)
	if infer_numeric(x) and infer_numeric(y) and len(set(x)) > threshold:
		new_x = [x[i] for i in range(len(x)) if is_numeric(x[i]) and is_numeric(y[i]) and not np.isnan(x[i]) and not np.isnan(y[i])]
		new_y = [y[i] for i in range(len(x)) if is_numeric(x[i]) and is_numeric(y[i]) and not np.isnan(x[i]) and not np.isnan(y[i])]
		x = new_x
		y = new_y
		if len(x) >= threshold and len(y) >= threshold:
			if mode.lower() in ["sm", "spearman"]:
				sm = scipy.stats.spearmanr(x, y)
			else:
				sm = scipy.stats.pearsonr(x,y)
			return({"Correlation":sm.correlation, "p-value":sm.pvalue})
	elif not infer_numeric(x) and not infer_numeric(y) and len(set(x)) > threshold:
		new_x = [x[i] for i in range(len(x)) if x[i] not in forbidden and y[i] not in forbidden]
		new_y = [y[i] for i in range(len(x)) if x[i] not in forbidden and y[i] not in forbidden]
		x = new_x
		y = new_y
		if len(x) >= threshold and len(y) >= threshold:
