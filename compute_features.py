...
def rc_enrichment(seqs, genename, space="-", alphabet=["T","C", "A", "G", "t", "c", "a", "g"], path=ram_disk, quiet=True):
	...
	align_codons.align_codons(path+"tmp.fasta", output=path+"tmp.fasta")
...
def parse_netOglyc(ORF):
	aaseq = translate_seq(ORF)
	response = tools.retry_request(requests.post, ["http://www.cbs.dtu.dk/cgi-bin/webface2.fcgi"], {"data":{"configfile":"/usr/opt/www/pub/CBS/services/NetOGlyc-4.0/NetOGlyc.cf", "SEQPASTE":aaseq, "SEQSUB":"(binary)"}})
	jobid = max(re.findall("name=\"jobid\" value=\"(\S+)\"", response.text), key=len)
	timer = 20
	while '<b>Explain</b></a> the output.  Go <a\nhref="javascript:history.back()"><b>back</b></a>.</font>' not in str(response.text) and timer >= 0:
		time.sleep(20)
...
def parse_seqs(seqdir, genename, space="-"):
	...
	seqs = tools.read_fasta(f, aformat="WHOLE")
	sorted_seqs = sorted(seqs.items(), key=lambda kv: len(kv[1].replace(space, "")))
...
def precompute(genename, seqs, ids={}, taxid=9606, path=ram_disk, outdir="./gene_data/", space="-", quiet=False):
	...
	try:
		p_aa = Process(target=run_blast, args=(seqs["ORF"], genename, "aa", "nr", True, None, path, space))
		p_nt = Process(target=run_blast, args=(seqs["ORF"], genename, "nt", "nt", True, None, path, space))
		p_aa.start()
		p_nt.start()
	except Exception as e:
		tb = traceback.format_exc()
	...
	try:
		...
		df.index += 1
		df.to_csv(outdir + genename + ".tsv", sep="\t")
	except Exception as e:
		tb = traceback.format_exc()
		print("\033[93m" + str(tb) + "\033[0m")
		print("\033[93m" + str(e) + "\033[0m")
...
def compute_features(mutfile, codonfile, codonpairfile, seqdir, space="-"):
	...
	muts = muts[[col for col in muts.columns if "Unnamed" not in str(col)]].sample(frac=1) #randomly reorder rows and remove "Unnamed" columns
	...
	try:
		for j, (k, row) in enumerate(muts.iterrows()):
			if os.path.isfile(seqdir + row[gene] + ".tsv"):
				row = dict(row)
				if row[gene] != last_gene:
					seqs = parse_seqs(seqdir, row[gene], space=space)
					last_gene = row[gene]
				wtseq = seqs["ORF"]
				...
				data[mutid]["Position in codon"] = posincodon + 1
				...
				genedata = pd.read_csv(seqdir + row[gene] + ".tsv", sep="\t", header=0, index_col=0)
				for col in genedata:
					if col in ntfeats:
		...
		df = pd.DataFrame(data)
		pd.DataFrame(data).T.to_csv("./mutdata_computed_part_" + str(num-1) + ".tsv", sep="\t")
		print(e)
		traceback.print_exc()
...
if __name__ == '__main__':
	...
	for f in os.listdir(clargs.seqdir):
		if f.endswith(".fasta") or f.endswith(".fa") and "aligned" not in f:
			seqs = parse_seqs(clargs.seqdir, os.path.splitext(f)[0])
